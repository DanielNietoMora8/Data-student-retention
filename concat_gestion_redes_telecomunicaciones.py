...
def detectar_columna_programa(df: pd.DataFrame) -> str | None:
    ...
    for col in columnas:
        ...
        if serie.str.contains("GESTIÓN DE REDES DE TELECOMUNICACIONES", na=False).any():
            return col
...
def main() -> None:
    ...
    for ruta in archivos:
        ...
        mask_ds = serie_prog.str.contains("GESTIÓN DE REDES DE TELECOMUNICACIONES", na=False)
        df_ds = df.loc[mask_ds].copy()

        n_ds = len(df_ds)
        print(f"  Registros filtrados para Desarrollo de Software (contiene 'GESTIÓN DE REDES DE TELECOMUNICACIONES'): {n_ds}")
    ...
    if not dfs_filtrados:
        print("\nNo se encontraron registros de programas que contengan 'GESTIÓN DE REDES DE TELECOMUNICACIONES' en ningún archivo.")
        return

    df_total = pd.concat(dfs_filtrados, ignore_index=True)
    salida = carpeta_dc / "Gestion_Redes_Telecomunicaciones_Todos.xlsx"
